...
type Family = {
  ...
  tiles: Tile[];
  nonDecorative: Tile[];
  byRole: Map<EditorTerrainTileRole, Tile[]>;
};
...
export async function generateBasicGroundTerrain(options: BasicTerrainGenerationOptions): Promise<EditorTilePlacement[]> {
  ...
  appendDecorativePlacements(placements, cells, gridSize, seed + 613, counters, maxPlacements, decorativeFamilies);
  return placements;
}
...
function createCells(options: {
  ...
  roadMask: Mask;
  baseFamilies: Family[];
  waterFamilies: Family[];
  roadFamilies: Family[];
}): Cell[][] {
  const baseRegions = createRegions(options.baseFamilies, options.columns, options.rows, options.seed + 17, 1.45);
  ...
  applyBiomePass(cells, options.baseFamilies, options.seed + 733);
  applyTransitionPass(cells, options.baseFamilies, options.seed + 809);
  smoothBase(cells, 2);
  return cells;
}

function applyBiomePass(cells: Cell[][], baseFamilies: Family[], seed: number): void {
  ...
  if (baseFamilies.length <= 1) return;
  ...
  forEachCell(cells, (cell, column, row) => {
    ...
    const roughness = layeredNoise(column - 57, row + 83, seed + 337, 18, 7, 3);

    let pool: Family[] = grass.length > 0 ? grass : baseFamilies;
    if (roughness > 0.78 && rock.length > 0) pool = rock;
    else if (dryness > 0.68 && sand.length > 0) pool = sand;
    else if (moisture < 0.34 && dirt.length > 0) pool = dirt;
    else if (moisture < 0.48 && dirt.length > 0 && normalizedNoise(column, row, seed + 29) > 0.35) pool = dirt;
...
function applyTransitionPass(cells: Cell[][], baseFamilies: Family[], seed: number): void {
  ...
  forEachCell(cells, (cell, column, row) => {
    ...
    if (roadDistance === 2 && dirt.length > 0 && normalizedNoise(Math.floor(column / 2), Math.floor(row / 2), seed + 73) > 0.28) {
      const pool = grass.length > 0 && normalizedNoise(column, row, seed + 79) > 0.72 ? grass : dirt;
...
function appendDecorativePlacements(placements: EditorTilePlacement[], cells: Cell[][], gridSize: number, seed: number, counters: Map<string, Map<EditorTerrainTileRole | 'all', number>>, max: number, families: Family[]): void {
  void counters;
...
function createRegions(families: Family[], columns: number, rows: number, seed: number, density: number): Region[] {
  if (families.length === 0) return [];
  if (families.length === 1) return [{ family: families[0], column: columns / 2, row: rows / 2, radiusBias: 1 }];
  const count = Math.max(families.length, Math.min(families.length * 3, Math.round(Math.sqrt(columns * rows) / 18 * density)));
  const regions: Region[] = [];
  for (let index = 0; index < count; index += 1) regions.push({ family: families[index % families.length], column: normalizedNoise(seed + index * 17, index * 31 + 3, seed) * columns, row: normalizedNoise(seed + index * 29, index * 37 + 7, seed + 101) * rows, radiusBias: 0.75 + normalizedNoise(seed + index * 41, index * 43 + 11, seed + 203) * 0.6 });
  return regions;
}

function pickFamilyFromRegions(regions: Region[], fallback: Family[], column: number, row: number, seed: number): Family {
  if (regions.length === 0) return fallback[0];
...
function createIslandMask(columns: number, rows: number, seed: number): Mask { const values = new Set<string>(); const centerX = (columns - 1) / 2; const centerY = (rows - 1) / 2; const radiusX = Math.max(2, columns * 0.46); const radiusY = Math.max(2, rows * 0.46); const vertexCount = 10; const radii = Array.from({ length: vertexCount }, (_, index) => 0.92 + seededNoise(index, seed * 0.01, seed + 17) * 0.08); for (let row = 0; row < rows; row += 1) { for (let column = 0; column < columns; column += 1) { const nx = (column - centerX) / radiusX; const ny = (row - centerY) / radiusY; const distance = Math.sqrt(nx * nx + ny * ny); let angle = Math.atan2(ny, nx); if (angle < 0) angle += Math.PI * 2; const segment = (angle / (Math.PI * 2)) * vertexCount; const index = Math.floor(segment) % vertexCount; const allowedRadius = lerp(radii[index], radii[(index + 1) % vertexCount], smoothStep(segment - Math.floor(segment))); if (distance <= allowedRadius) values.add(`${column}:${row}`); } } removeTinyIslands(values, columns, rows); return setMask(columns, rows, values); }
function createWaterMask(columns: number, rows: number, seed: number, landMask: Mask): Mask { const values = new Set<string>(); if (seededNoise(seed, seed + 13, seed + 29) > 0.25) createLake(values, columns, rows, seed, landMask); else createRiver(values, columns, rows, seed, landMask); removeTinyIslands(values, columns, rows); return setMask(columns, rows, values); }
function createLake(values: Set<string>, columns: number, rows: number, seed: number, landMask: Mask): void { const count = Math.min(2, Math.max(1, Math.floor(Math.min(columns, rows) / 70) + 1)); for (let index = 0; index < count; index += 1) paintEllipse(values, landMask, Math.round(columns * (0.3 + normalizedNoise(seed + index * 11, 7, seed) * 0.4)), Math.round(rows * (0.3 + normalizedNoise(seed + index * 17, 13, seed) * 0.4)), Math.max(3, Math.round(columns * (0.045 + normalizedNoise(seed, index + 23, seed) * 0.035))), Math.max(3, Math.round(rows * (0.045 + normalizedNoise(seed + 31, index, seed) * 0.035)))); }
function createRiver(values: Set<string>, columns: number, rows: number, seed: number, landMask: Mask): void { const horizontal = seededNoise(seed, seed + 13, seed + 29) > 0; const width = Math.max(1, Math.round(Math.min(columns, rows) * 0.025)); const start = horizontal ? { column: 0, row: Math.round(rows * (0.25 + normalizedNoise(seed, 1, seed) * 0.5)) } : { column: Math.round(columns * (0.25 + normalizedNoise(seed, 2, seed) * 0.5)), row: 0 }; const end = horizontal ? { column: columns - 1, row: Math.round(rows * (0.25 + normalizedNoise(seed, 3, seed) * 0.5)) } : { column: Math.round(columns * (0.25 + normalizedNoise(seed, 4, seed) * 0.5)), row: rows - 1 }; const a = horizontal ? { column: Math.round(columns * 0.33), row: Math.round(rows * (0.2 + normalizedNoise(seed, 5, seed) * 0.6)) } : { column: Math.round(columns * (0.2 + normalizedNoise(seed, 6, seed) * 0.6)), row: Math.round(rows * 0.33) }; const b = horizontal ? { column: Math.round(columns * 0.66), row: Math.round(rows * (0.2 + normalizedNoise(seed, 7, seed) * 0.6)) } : { column: Math.round(columns * (0.2 + normalizedNoise(seed, 8, seed) * 0.6)), row: Math.round(rows * 0.66) }; paintCubic(values, landMask, start, a, b, end, width); }
function createRoadMask(columns: number, rows: number, seed: number, landMask: Mask, waterMask: Mask): Mask { const values = new Set<string>(); const horizontal = seededNoise(seed, seed + 5, seed + 11) > 0; const width = Math.max(1, Math.round(Math.min(columns, rows) * 0.012)); const center = { column: Math.round(columns * (0.45 + normalizedNoise(seed, 19, seed) * 0.1)), row: Math.round(rows * (0.45 + normalizedNoise(seed, 29, seed) * 0.1)) }; const start = horizontal ? { column: 0, row: Math.round(rows * (0.3 + normalizedNoise(seed, 31, seed) * 0.4)) } : { column: Math.round(columns * (0.3 + normalizedNoise(seed, 37, seed) * 0.4)), row: 0 }; const end = horizontal ? { column: columns - 1, row: Math.round(rows * (0.3 + normalizedNoise(seed, 41, seed) * 0.4)) } : { column: Math.round(columns * (0.3 + normalizedNoise(seed, 43, seed) * 0.4)), row: rows - 1 }; paintStepped(values, landMask, waterMask, start, center, width); paintStepped(values, landMask, waterMask, center, end, width); paintDisc(values, landMask, center.column, center.row, Math.max(width + 1, 2), waterMask); return setMask(columns, rows, values); }
function paintStepped(values: Set<string>, landMask: Mask, avoidMask: Mask, start: Point, end: Point, width: number): void { let column = start.column; let row = start.row; const guard = Math.max(1, landMask.columns + landMask.rows) * 3; for (let step = 0; step < guard && (column !== end.column || row !== end.row); step += 1) { paintDisc(values, landMask, column, row, width, avoidMask); const dx = end.column - column; const dy = end.row - row; if (Math.abs(dx) >= Math.abs(dy)) column += Math.sign(dx); else row += Math.sign(dy); } paintDisc(values, landMask, end.column, end.row, width, avoidMask); }
