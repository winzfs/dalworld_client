import type { EditorTerrainMaterial, EditorTerrainMovementMode, EditorTerrainRuleSet, EditorTerrainTileRole, EditorTilePlacement, EditorTilesetAsset } from '../types';
...
function ruleFits(rule: EditorTerrainRuleSet['rules'][number], scale: number, grid: number): boolean { return Math.round(rule.sourceRect.width * scale) === grid && Math.round(rule.sourceRect.height * scale) === grid; }
...
function distance(c: number, r: number, p: Point): number { return Math.hypot(c - p.column, r - p.row); }
