...
type TerrainDiagnostic = { severity: TerrainDiagnosticSeverity; message: string };
...
export class TerrainGeneratorPanel {
  ...
  constructor(private readonly options: TerrainGeneratorPanelOptions) {
    ...
    this.closeButton.addEventListener('click', (event) => { stopEvent(event); this.close(); });
    ...
    addButton.onclick = () => { this.options.onAddCurrentTileset(); this.render(); };
    ...
    this.body.append(
      help,
      addButton,
      row(label('생성 형태'), this.shapeSelect),
      row(label('시드'), this.seedInput, randomSeedButton),
      ruleButton,
      this.list,
      this.diagnostics,
      generateButton,
  ...
  render(): void {
    ...
    for (const asset of tilesets) this.list.appendChild(this.createTilesetRow(asset));
    void this.refreshDiagnostics();
  }

  private createTilesetRow(asset: EditorTilesetAsset): HTMLElement {
    const item = document.createElement('div');
    item.style.cssText = ['display:flex', 'align-items:center', 'gap:8px', 'padding:7px', 'border:1px solid rgba(255,255,255,.12)', 'border-radius:10px', 'background:rgba(255,255,255,.05)'].join(';');
    const preview = document.createElement('span');
    preview.style.cssText = ['display:inline-block', 'width:28px', 'height:28px', 'flex:0 0 28px', 'border-radius:6px', 'background-color:rgba(255,255,255,.08)', `background-image:url(${asset.url})`, 'background-position:center', 'background-repeat:no-repeat', 'background-size:contain', 'image-rendering:pixelated'].join(';');
    ...
    removeButton.addEventListener('pointerdown', stopEvent);
  ...
  private async requestGenerate(): Promise<void> {
    ...
    if (important.length > 0) {
      ...
      if (!window.confirm(`지형 규칙 진단 경고가 있습니다.\n\n${preview}${suffix}\n\n그래도 생성할까요?`)) return;
    }
  ...
  private configureShapeSelect(): void {
    ...
    this.shapeSelect.append(createShapeOption('rect', '사각형'), createShapeOption('island', '섬/덩어리'));
    this.shapeSelect.value = readStoredShape(this.mapName);
    this.shapeSelect.onchange = () => writeStoredShape(this.mapName, this.shapeSelect.value as TerrainGenerationShape);
  }
  ...
  private get mapName(): string { return this.options.mapName ?? 'dalworld-map'; }
  ...
  private async ensureRuleManagerPanel(): Promise<any> {
    ...
    const [panelModule] = await Promise.all([import('./TerrainRuleManagerPanel'), this.ensureRuleStorage()]);
    ...
    this.ruleManagerPanel = new panelModule.TerrainRuleManagerPanel({
      ...
      onSaveTilesetMaterial: (asset: EditorTilesetAsset, material: EditorTerrainMaterial, movementMode: EditorTerrainMovementMode, scale: number) => this.saveTilesetMaterial(asset, material, movementMode, scale),
    });
  ...
  private renderDiagnostics(diagnostics: TerrainDiagnostic[]): void {
    ...
    header.textContent = `규칙 진단 · 오류 ${diagnostics.filter((item) => item.severity === 'error').length} · 경고 ${diagnostics.filter((item) => item.severity === 'warning').length}`;
    ...
    for (const item of visible) {
      ...
      itemRow.textContent = `${severityIcon(item.severity)} ${item.message}`;
      itemRow.style.cssText = diagnosticRowStyle(item.severity);
  ...
  private saveTilesetMaterial(asset: EditorTilesetAsset, material: EditorTerrainMaterial, movementMode: EditorTerrainMovementMode, scale: number): void {
    if (!this.ruleStorage) return;
    this.ruleSet = this.ruleStorage.upsertTilesetMaterial(asset, material, movementMode, scale);
    this.ruleManagerPanel?.render?.();
    void this.refreshDiagnostics();
  }
...
function diagnoseTerrainRules(tilesets: EditorTilesetAsset[], ruleSet: EditorTerrainRuleSet): TerrainDiagnostic[] {
  ...
  for (const asset of tilesets) {
    ...
    if (hasAnyRule && rules.length === 0) { diagnostics.push({ severity: 'warning', message: `${asset.name}: 등록된 규칙이 없어 생성에 참여하지 않을 수 있습니다.` }); continue; }
    if (rules.length === 0) { diagnostics.push({ severity: 'info', message: `${asset.name}: 규칙 없음 · 전체 타일셋 fallback 사용 가능` }); continue; }
    if (nonDecorative.length === 0) { diagnostics.push({ severity: 'error', message: `${asset.name}: decorative만 있고 center/edge 규칙이 없습니다.` }); continue; }
    if (!roles.has('center')) diagnostics.push({ severity: settings.material === 'water' || settings.material === 'road' ? 'error' : 'warning', message: `${asset.name}: center 규칙이 없습니다.` });
    const missingEdges = REQUIRED_EDGE_ROLES.filter((role) => !roles.has(role));
    if (missingEdges.length > 0) diagnostics.push({ severity: 'warning', message: `${asset.name}: edge 규칙 부족 (${missingEdges.length}/4 없음)` });
    const missingOuterCorners = REQUIRED_OUTER_CORNER_ROLES.filter((role) => !roles.has(role));
    if (missingOuterCorners.length > 0) diagnostics.push({ severity: 'warning', message: `${asset.name}: outer corner 규칙 부족 (${missingOuterCorners.length}/4 없음)` });
    const missingInnerCorners = REQUIRED_INNER_CORNER_ROLES.filter((role) => !roles.has(role));
    if (missingInnerCorners.length === REQUIRED_INNER_CORNER_ROLES.length) diagnostics.push({ severity: 'info', message: `${asset.name}: inner corner 규칙이 없습니다. 복잡한 물가/길 안쪽 모서리는 center fallback 됩니다.` });
    if ((settings.material === 'water' || settings.material === 'road') && nonDecorative.length < 5) diagnostics.push({ severity: 'warning', message: `${asset.name}: ${settings.material} 규칙이 적어 단일 타일처럼 보일 수 있습니다.` });
    diagnostics.push({ severity: 'info', message: `${asset.name}: 타일셋 scale ${settings.scale} · 16px 규칙은 ${16 * settings.scale}px 그리드용` });
  }
  ...
  if ((materialCounts.get('sand') ?? 0) === 0 && (materialCounts.get('dirt') ?? 0) === 0) diagnostics.push({ severity: 'info', message: 'sand/dirt 타일셋이 없어 물가/길가 완충지대가 grass 계열로 남을 수 있습니다.' });
  return diagnostics.length > 0 ? diagnostics : [{ severity: 'info', message: '필수 규칙 진단 통과' }];
}

function getTilesetSettings(asset: EditorTilesetAsset, ruleSet: EditorTerrainRuleSet): { material: EditorTerrainMaterial; movementMode: EditorTerrainMovementMode; scale: number } {
  ...
  return { material, movementMode: saved?.movementMode ?? getDefaultMovementMode(material), scale: normalizeScale(saved?.scale) };
}
...
function createShapeOption(value: TerrainGenerationShape, labelText: string): HTMLOptionElement { const option = document.createElement('option'); option.value = value; option.textContent = labelText; return option; }
function shapeStorageKey(mapName: string): string { return `dalworld:editor-terrain-shape:${mapName}`; }
function seedStorageKey(mapName: string): string { return `dalworld:editor-terrain-seed:${mapName}`; }
function readStoredShape(mapName: string): TerrainGenerationShape { const raw = window.localStorage.getItem(shapeStorageKey(mapName)); return raw === 'island' ? 'island' : 'rect'; }
function writeStoredShape(mapName: string, shape: TerrainGenerationShape): void { window.localStorage.setItem(shapeStorageKey(mapName), shape); window.localStorage.setItem('dalworld:editor-terrain-shape:dalworld-map', shape); }
function readStoredSeed(mapName: string): number { const raw = window.localStorage.getItem(seedStorageKey(mapName)); return normalizeSeed(raw ? Number(raw) : 1); }
function writeStoredSeed(mapName: string, seed: number): void { const normalized = normalizeSeed(seed); window.localStorage.setItem(seedStorageKey(mapName), String(normalized)); window.localStorage.setItem('dalworld:editor-terrain-seed:dalworld-map', String(normalized)); }
function normalizeSeed(seed: number): number { if (!Number.isFinite(seed)) return 1; return Math.max(0, Math.min(999_999_999, Math.round(seed))); }
function normalizeScale(value: number | undefined): number { if (!Number.isFinite(value) || (value as number) <= 0) return 1; return Math.max(0.1, Math.min(10, Math.round((value as number) * 10) / 10)); }
function severityIcon(severity: TerrainDiagnosticSeverity): string { if (severity === 'error') return '⛔'; if (severity === 'warning') return '⚠️'; return 'ℹ️'; }
function diagnosticRowStyle(severity: TerrainDiagnosticSeverity): string { const base = 'padding:6px 7px;border-radius:9px;line-height:1.35;border:1px solid;'; if (severity === 'error') return `${base}color:#fecaca;background:rgba(127,29,29,.34);border-color:rgba(248,113,113,.38);`; if (severity === 'warning') return `${base}color:#fde68a;background:rgba(120,53,15,.28);border-color:rgba(251,191,36,.32);`; return `${base}color:rgba(224,242,254,.82);background:rgba(14,116,144,.18);border-color:rgba(125,211,252,.22);`; }
function stopEvent(event: Event): void { event.preventDefault(); event.stopPropagation(); }
function row(...children: HTMLElement[]): HTMLDivElement { const element = document.createElement('div'); element.style.cssText = 'display:flex;align-items:center;gap:8px;'; element.append(...children); return element; }
function label(text: string): HTMLSpanElement { const element = document.createElement('span'); element.textContent = text; element.style.cssText = 'width:58px;flex:0 0 58px;color:rgba(248,250,252,.72);font-weight:800;'; return element; }
...
function compactButtonStyle(): string { return 'border:1px solid rgba(255,255,255,.18);border-radius:8px;background:rgba(255,255,255,.08);color:#f8fafc;padding:7px 8px;cursor:pointer;font-weight:800;'; }
function primaryButtonStyle(): string { return 'border:1px solid rgba(56,189,248,.42);border-radius:10px;background:rgba(14,165,233,.22);color:#f8fafc;padding:9px 10px;cursor:pointer;font-weight:900;'; }
function secondaryButtonStyle(): string { return 'border:1px solid rgba(167,139,250,.45);border-radius:10px;background:rgba(109,40,217,.28);color:#f8fafc;padding:8px 10px;cursor:pointer;font-weight:900;'; }
function selectStyle(): string { return 'flex:1 1 auto;min-width:0;border:1px solid rgba(255,255,255,.16);border-radius:8px;background:rgba(0,0,0,.28);color:#f8fafc;padding:7px 8px;font-weight:800;'; }
function seedInputStyle(): string { return 'flex:1 1 auto;min-width:0;border:1px solid rgba(255,255,255,.16);border-radius:8px;background:rgba(0,0,0,.28);color:#f8fafc;padding:7px 8px;font-weight:800;'; }
...
function emptyBoxStyle(): string { return 'padding:8px;border:1px dashed rgba(255,255,255,.16);border-radius:10px;color:rgba(248,250,252,.58);'; }
function formatError(error: unknown): string { if (error instanceof Error) return error.message; return String(error); }
